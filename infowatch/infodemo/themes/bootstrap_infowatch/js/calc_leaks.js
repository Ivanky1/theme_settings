...
$("#go_baby").click(function(){
        ...
        in_default[4] = Math.abs(parseInt($("#form_0_1").val()))+ Math.abs(parseInt($("#form_0_2").val()));
        in_default[5] = Math.abs(parseInt($("#form_0_1").val()));             
        in_default[6] = Math.abs(parseInt($("#form_0_2").val()));
        in_default[7] = Math.abs(parseInt($("#form_0_2").val()));
        in_default[8] = Math.abs(parseInt($("#form_0_2").val()));
        in_default[9] = Math.abs(parseInt($("#form_0_4").val()));
...
drugie_reag["ur_itogo1"]= [
    ...
    var vrem_ur_presledovanie1 =[];
        ...
        var summa = (reag1_max+koof_real_bred);
         $("#vivod").append("<div class='block_bl'>Затраты на реагирование, устранение и расследование </div><div class='block_bl result_nombers'><strong>"+ (reag1_max+koof_real_bred) +"</strong> руб.</div>");
        ...
        summa = summa+ shtraf1_max;
         $("#vivod").append("<div class='block_bl'>Штрафы и другие санкции регуляторов </div><div class='block_bl result_nombers'><strong>"+ shtraf1_max  +"</strong> руб.<a class='block_bl quest' href='#pole14' style='margin-right: -17px;margin-top: 0;margin-left: 10px;'>&nbsp;</a></div>");
         $("#vivod").append("<div style='clear:both'></div>");      
        for (var a=0; a<7; a++) {  
        ...
        summa = summa+ ((reputacia1_max+reputacia1_alone)+(in_default[11]*in_default[20]*in_default[8]+in_default[12]*in_default[21]*in_default[9])*in_default[13]);
            $("#vivod").append("<div class='block_bl'>Репутационные потери и упущенная выгода </div><div class='block_bl quest q1'>&nbsp;</div><div class='block_bl result_nombers'><strong>"+ ((reputacia1_max+reputacia1_alone)+(in_default[11]*in_default[20]*in_default[8]+in_default[12]*in_default[21]*in_default[9])*in_default[13])  +"</strong> руб.</div>");        
         $("#vivod").append("<div style='clear:both;padding-top:34px;'></div>"); 
         $("#vivod").append("<div class='block_bl result_summa'><strong>Всего:</strong> </div><div class='block_bl result_summa'><strong>"+ summa +"</strong> руб.</div>");
       /*  $("#vivod").append("<div style='clear:both'></div>"); 
        for (var a=0; a<7; a++) {  
        ...
        $(".label_for_this").on("click", "a.quest", function () {
            ...
            $(link_help).addClass("active");
            $(this).parent().parent().parent(".clerix").append($(link_help));
            console.log($(".clerix .cbox.active"));
            return false;
        });    
