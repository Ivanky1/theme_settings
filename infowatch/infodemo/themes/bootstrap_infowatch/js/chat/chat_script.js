...
// Предчатовая форма. Структура: {<поле>: {type: <тип поля>, required: <обязательно ли поле>, validator: <регулярка>}}
        var iwc_prechat_form = {
            ...
            submit: {type: 'submit', value: 'Погнали'}
        };
...
// Создаёт в "#prechat_form" предчатовую форму
        function GenerateForm() {
            ...
            jQuery.each(iwc_prechat_form_order, function(_, field) {
                ...
                jQuery.each('type, value'.split(', '), function(_, attr) {
                    $html.attr(attr, elem[attr]);
                });
                ...
                if (elem.type == 'select' || elem.type == 'datalist') {
                    ...
                    if (elem.type == 'datalist') {
                        console.log('Appending datalist: ', $options_container);
...
// Функция отправки предчатовой формы. Проверяет значения и инициализирует чат.
        function PrechatSubmit() {
            console && console.log('Prechat submit started');
            ...
            console && console.log('Prechat form validation', values);
            ...
            console && console.log('Prechat filling fields');

            liveagent.addCustomDetail('E-mail', values.email);
            liveagent.addCustomDetail('Имя', values.name);
            liveagent.addCustomDetail('Фамилия', values.surname);
            liveagent.addCustomDetail('Телефон', values.phone);
            liveagent.addCustomDetail('Номер обращения', values.case_no || 'нет');
            liveagent.addCustomDetail('Страна', values.country);
            liveagent.addCustomDetail('Продукт', values.product);
            liveagent.addCustomDetail('Версия', values.version);
            liveagent.addCustomDetail('Серьёзность', values.severity);
            liveagent.addCustomDetail('Тема запроса', values.summary);
            liveagent.addCustomDetail('Описание проблемы', values.description);

            window.iw_liveagent_form_values = values;

            /*  liveagent.addCustomDetail('auto_Case_RecordType', 'Support case', false);
             liveagent.addCustomDetail('auto_Case_Origin', 'Поддержка: чат', false);*/
            ...
            liveagent.findOrCreate('Case')
                ...
                /*      .map('RecordType', 'auto_Case_RecordType', false, false, true)
                 .map('Origin', 'auto_Case_Origin', false, false, true)
            ...
            console && console.log('Prechat initializing live agent');
            ...
            console && console.log('Prechat submission done');
        }

        jQuery(function($) {
            console.log('window.iw_liveagent_form_values');
            console.log(window.iw_liveagent_form_values);
