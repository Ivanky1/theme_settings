...
<?php if ($page == 1){ ?>
    ...
    <div class="content clearfix"<?php print $content_attributes; ?>>
        ...
        if (isset($node->field_open_private_material[LANGUAGE_NONE]) && $node->field_open_private_material[LANGUAGE_NONE][0]['value'] == 'Открыть') {
            $file = field_get_items('node', $node, 'field_file_private_material');
            $field_file = field_view_value('node', $node, 'field_file_private_material', $file[0], ['class'=>'mbot36']);
            echo  "<div class='mbot36'>".render($field_file)."</div>";
        } else {
            if (!variable_get('profile_anonim_email') && $user->uid == 0) {
                ...
                echo "<h2 class='cg mtop36 mbot12'>Скачайте материал:</h2>";
